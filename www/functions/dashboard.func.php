...
function test_billet_existant($professeur, $etudiant, $module, $date){
    ...
    $u = [
        ...
        $sql = "SELECT COUNT(SIG_CODE) FROM ABS_SIGNALEMENT WHERE UTI_CODE = :UTI_CODE AND UTI_CODE_1 = (SELECT UTI_CODE FROM ABS_UTILISATEUR WHERE UTI_IDENTIFIANT = :UTI_IDENTIFIANT) AND COU_CODE = :COU_CODE AND SIG_DATE = :SIG_DATE";
        $req = $db->prepare($sql);
        $req->execute($u);
    }
